...
<head>
    <title>Dokumen Tambahan: SPK {{ $spk->spk_code }}</title>
</head>

<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
        <h2 style="color: #2F80ED;">Ada Dokumen Tambahan</h2>

        <p>Halo <strong>{{ $recipient->name }}</strong>,</p>

        <p>Ada dokumen tambahan yang perlu anda cek (sebanyak <strong>{{ $count }}</strong> dokumen) pada section <strong>{{ $sectionName }}</strong>.</p>
        ...
        <p style="margin-bottom: 20px;">Silakan klik tombol di bawah ini untuk melihat detail SPK dan mengunggah dokumen tersebut.</p>

        <a href="{{ config('app.url') }}/shipping/{{ $spk->id }}#section-{{ $spk->id_section_ref ?? '' }}"
