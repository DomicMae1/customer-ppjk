...
import { Calendar as CalendarIcon, ChevronDown, ChevronUp, CircleHelp, Play, Plus, Search } from 'lucide-react';
...
export default function CustomerForm({
    ...
    const shipmentData = {
        spkDate: '12/11/25 17.14 WIB',
        type: 'Export',
        siNumber: 'PO62R23094030R123T5',
    ...
    const handleFinalSave = () => {
        ...
        router.visit(`/shipping/1`, { replace: true, preserveState: false });
    };

    const handleDateChange = (sectionId: string, date: string) => {
    ...
    return (
        <div className="w-full max-w-md bg-white p-4 font-sans text-sm text-gray-900">
            ...
            <div className="mb-5 rounded-lg border border-gray-200 p-3 shadow-sm">
                <div className="font-bold text-black">SPK CREATED</div>
            ...
            <div className="mb-6 space-y-1 pl-1">
                ...
                <div className="flex gap-1">
                    ...
                    <div className="flex flex-col">
                        {shipmentData.hsCodes.map((item, index) => (
                            <div key={index} className="flex items-center gap-2">
                                <span>{item.code}</span>
                                <button className="font-bold text-blue-600 hover:underline">[INSW]</button>
                    ...
                    return (
                        <div key={section.id} className="rounded-lg border border-gray-200 px-1 transition-all">
                            ...
                            <div className="flex items-center justify-between px-3 py-3">
                                ...
                                {/* Tombol Edit (Hanya muncul jika section tertutup) */}
                                {!isOpen && (
                                    <Button
                                        onClick={() => handleEditSection(section.id)}
                                        className="h-7 rounded bg-black px-4 text-xs font-medium text-white hover:bg-gray-800"
                                    >
                                        edit
                                    </Button>
                                )}
                            ...
                            {isOpen && (
                                <div className="border-t border-gray-100 px-3 pt-2 pb-4">
                                    {/* --- ADDED: Set Deadline Date PER SECTION --- */}
                                    <div className="mb-6 space-y-2">
                                        ...
                                        <div className="relative">
                                            <Input
                                                ...
                                                value={deadlineDates[section.id] || ''}
                                                onChange={(e) => handleDateChange(section.id, e.target.value)}
                                            />
                                            <CalendarIcon className="pointer-events-none absolute top-1/2 right-3 h-5 w-5 -translate-y-1/2 text-black" />
                                        </div>
                                    </div>
