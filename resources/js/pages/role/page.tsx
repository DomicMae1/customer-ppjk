...
export default function ManageRoles() {
    ...
    const onSubmit = () => {
        ...
        const data = {
            id_perusahaan: selectedCompanyId,
            name: roleName,
    ...
    const filteredPermissions = Object.entries(permissions).filter(([model]) => model.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    const getPermissionColor = (name: string) => {
        ...
        switch (action) {
            ...
            case 'create':
                return 'text-emerald-500 bg-emerald-50 border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800';
            case 'update':
                return 'text-amber-500 bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800';
            case 'delete':
                return 'text-rose-500 bg-rose-50 border-rose-200 dark:bg-rose-900/20 dark:border-rose-800';
            default:
                return 'text-slate-500 bg-slate-50 border-slate-200 dark:bg-slate-900/20 dark:border-slate-800';
        }
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <Dialog open={openDelete} onOpenChange={setOpenDelete}>
                ...
                <DialogContent className="border-border bg-background text-foreground max-w-[90vw] rounded-2xl p-6 shadow-2xl sm:max-w-md">
                    <DialogHeader>
                    ...
                    <DialogFooter className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                        ...
                        <Button
                            type="button"
                            variant="destructive"
                            className="w-full font-bold shadow-lg shadow-rose-500/20 sm:w-auto"
            ...
            <Dialog open={openForm} onOpenChange={setOpenForm}>
                <DialogContent className="border-border bg-background/95 text-foreground flex max-h-[95vh] max-w-[95%] flex-col overflow-hidden rounded-3xl border-white/20 p-0 shadow-2xl backdrop-blur-xl sm:max-w-4xl">
                    ...
                    <div className="flex-1 space-y-8 overflow-y-auto p-6">
                        ...
                        <div className="space-y-3">
                            <Label htmlFor="roleName" className="text-foreground text-sm font-bold tracking-wider uppercase">
                            ...
                            <Input
                                ...
                                className="bg-background/50 border-input focus:ring-primary/20 h-12 rounded-xl text-lg transition-all focus:ring-2"
                            />
                            {errors?.name && (
                                <p className="mt-1 ml-1 text-[10px] font-bold tracking-tighter text-rose-500 uppercase">{errors.name}</p>
                            )}
                        </div>

                        {/* Role Type Dropdown (Added) */}
                        <div className="space-y-3">
                            <Label htmlFor="roleType" className="text-foreground text-sm font-bold tracking-wider uppercase">
                                {trans_role.label_role_type || 'Role Type'}
                            </Label>
                            <Select value={selectedRoleType} onValueChange={setSelectedRoleType}>
                                <SelectTrigger className="bg-background/50 border-input focus:ring-primary/20 h-12 rounded-xl text-lg transition-all focus:ring-2">
                            ...
                            {errors?.role_type && (
                                <p className="mt-1 ml-1 text-[10px] font-bold tracking-tighter text-rose-500 uppercase">{errors.role_type}</p>
                            )}
                        </div>

                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                <Label className="text-foreground text-sm font-bold tracking-wider uppercase">
                                    {trans_role.label_permissions || 'Permissions'}
                                </Label>
                                <div className="relative w-full max-w-xs">
                                    <Input
                                        ...
                                        className="bg-muted/50 h-9 rounded-full text-xs"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                {filteredPermissions.map(([model, modelPermissions]: [string, any]) => (
                                    <div
                                        key={model}
                                        className="group border-border bg-card/50 hover:bg-card flex flex-col gap-4 rounded-2xl border p-5 transition-all hover:shadow-md"
                                    >
                                        <div className="flex items-center justify-between">
                                            <h3 className="text-foreground flex items-center gap-2 text-base font-bold capitalize">
                                                <span className="bg-primary h-2 w-2 rounded-full" />
                                                {model.replace(/-/g, ' ')}
                                            </h3>
                                            <div className="bg-muted/50 flex items-center gap-2 rounded-full border border-white/10 px-3 py-1.5">
                                                <Checkbox
                                                    id={`select-all-${model}`}
                                                    ...
                                                    className="border-primary/50 data-[state=checked]:bg-primary rounded-sm"
                                                />
                                                <Label
                                                    htmlFor={`select-all-${model}`}
                                                    className="text-muted-foreground hover:text-foreground cursor-pointer text-[10px] font-bold tracking-tighter uppercase"
                                                >
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {modelPermissions.map((permission: string) => (
                                                <div
                                                    ...
                                                    className={`flex cursor-pointer items-center gap-3 rounded-xl border p-2 transition-all hover:scale-[1.02] active:scale-95 ${
                                                        selectedPermissions.includes(permission)
                                                            ? getPermissionColor(permission)
                                                            : 'bg-muted/20 text-muted-foreground border-transparent opacity-60 grayscale'
                                                    }`}
                                                >
                                                    <Checkbox id={permission} checked={selectedPermissions.includes(permission)} className="hidden" />
                    ...
                    <DialogFooter className="bg-muted/30 flex gap-3 border-t p-6 backdrop-blur-md">
                        <DialogClose asChild>
                            <Button type="button" variant="ghost" className="h-12 rounded-xl px-8 font-bold">
                                {trans_role.btn_cancel || 'Cancel'}
                            </Button>
                        </DialogClose>
                        <Button
                            type="button"
                            onClick={onSubmit}
                            className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-primary/20 h-12 rounded-xl px-10 text-base font-black shadow-xl transition-all hover:-translate-y-0.5 active:translate-y-0"
                        >
